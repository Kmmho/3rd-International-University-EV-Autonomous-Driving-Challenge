...
class TrajectoryOptimization():
    ...
    def set_global(self):
        ...
        self.obstacle_generate()
        self.V_g = vel * np.ones(self.num_local_waypoint)
        self.yaw_g = np.ones(self.num_local_waypoint) * np.pi/4

        self.u_g = self.V_g * np.cos(self.yaw_g)
        self.v_g = self.V_g * np.sin(self.yaw_g)

        ### Global Path의 좌표값들이 들어오면 됨. ###
        self.x_g = np.arange(0, self.horizon_time * self.u_g[0] + self.u_g[0] * self.t_diff[0], self.t_diff[0] * self.u_g[0])
        self.y_g = np.arange(0, self.horizon_time * self.v_g[0] + self.v_g[0] * self.t_diff[0], self.t_diff[0] * self.v_g[0])
    ...
    def cost_function(self, x):
        ...
        speed_cost = sum((self.u_d - u_a)**2 + (self.v_d - v_a)**2) / (((np.mean(self.u_d))**2 + np.mean(self.v_d)**2))
    ...
    def calculate_trajectory_state(self, C_u, C_v):
        ...
        psi = np.array([math.atan2(v[k], u[k]) for k in range(len(v))])
    ...
    def plot_trajectory(self, x, y):
        if not hasattr(self, 'ax'):
            ...
            self.ax.set_xlim(0, 7000)
            self.ax.set_ylim(0, 7000)
            
        # 장애물 정보를 저장하는 리스트
        obs = [[self.obstacle.obs_x[i], self.obstacle.obs_y[i], self.obstacle.obs_R[i]] for i in range(self.obstacle.obs_x.size)]
        
        # 장애물을 그래프에 추가
        obstacles = []
        for (obs_x, obs_y, r) in obs:
            obstacle = patches.Circle((obs_x, obs_y), r, edgecolor='black', fill=True)
        ...
        self.ax.plot(self.x_g, self.y_g, 'r--')
        self.ax.plot(self.past_x, self.past_y, 'b--')
    ...
    ###################### Initial Setting #########################
        ...
        self.y_d = self.y_g[0:self.num_local_waypoint]
        ...
        ## Planning이 발동되는 시점에서의 속도 값 ##
        self.C_u[3::] = 20 * np.cos(self.yaw_0)
        self.C_v[3::] = 20 * np.sin(self.yaw_0)
    ...
    ##################################################################
        ...
        while True:
            ...
            self.C_u[1] = u_0_dot * self.horizon_time/6 + self.C_u[0]
            self.C_v[1] = v_0_dot * self.horizon_time/6 + self.C_v[0]
            
            self.C_u[2] = u_0_dot_dot * self.horizon_time**2/30 - self.C_u[0] + 2*self.C_u[1]
            self.C_v[2] = v_0_dot_dot * self.horizon_time**2/30 - self.C_v[0] + 2*self.C_v[1]
            ...
            if self.obstacle.num_obs == 0:
                if x[-1] >= 5000:
                    break
            else:
                if x[0] >= self.obstacle.obs_x[-1] + self.obstacle.obs_R[-1] * 30 or len(x) > 1000:
                    break

            
            # Update States for next Step 
            u_0 = u[self.num_trav_waypoint-1]
            v_0 = v[self.num_trav_waypoint-1]

            u_0_dot = u_dot[self.num_trav_waypoint-1]
            v_0_dot = v_dot[self.num_trav_waypoint-1]

            u_0_dot_dot = u_dot_dot[self.num_trav_waypoint-1]
            v_0_dot_dot = v_dot_dot[self.num_trav_waypoint-1]
            ...
            # Residue States(psi) are not changed
            # self.v_g, self.V_g is changed to Real-time velocity !
            self.u_g = self.get_velocity() * np.ones(self.num_local_waypoint) * np.cos(self.yaw_g[0])
            self.v_g = self.get_velocity() * np.ones(self.num_local_waypoint) * np.sin(self.yaw_g[0])
            ...
            # Residue States(psi) are not changed
            # self.v_d, self.V_d is changed to Real-time velocity !
            self.u_d = self.u_g
            self.v_d = self.v_g
            self.x_d = self.x_g
            self.y_d = self.y_g
